...
ra_cal = [ra_cas,ra_cyg]
dec_cal = [dec_cas,dec_cyg]
...
#flux_cas = 41093.75*scalef[0]
#flux_cyg = 28062.87*scalef[1] 

flux_cas = 40172.03*scalef[0]
flux_cyg = 27586.11*scalef[1]
print flux_cas, flux_cyg

#flagdata(vis = filename, mode='manual', spw='0:000;076') #for 01
#flagdata(vis = filename, mode='manual', spw='0:014;015;016;017;018;095;096;097') #for 03
#flagdata(vis = filename, mode='manual', spw='0:008;009') #for 05
# no channel flag for 07
...
cl.addcomponent(flux=flux_cas, dir='J2000 23h23m24.000s +58d48m54.000s', index=-0.77, spectrumtype='spectral index', freq='31.296MHz', label='CasA')
cl.addcomponent(flux=flux_cyg, dir='J2000 19h59m28.357s +40d44m02.097s', index=0.085, spectrumtype='spectral index', freq='31.296MHz', label='CygA')

cl.rename('calibrationsources_componentlist.cl')
cl.done()
clearcal(filename, addmodel=True)
ft(filename, complist='calibrationsources_componentlist.cl', usescratch=True)
bandpass(filename, '07_calibrationtable.bcal', refant='34', uvrange='>15lambda', fillgaps=1)
#applycal(vis = filename, gaintable=['03_calibrationtable.bcal'], interp='linear')
#clean(vis=filename, imagename='03_solution_briggs_no_iter', mode='mfs', imsize=[4096,4096],cell=['1.875arcmin'],niter=0,  weighting= 'briggs', robust =0, stokes='I')
