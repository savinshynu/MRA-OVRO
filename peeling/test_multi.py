...
#cas = {"flux":41093.75, "dir":"J2000 23h23m24.000s +58d48m54.000s","index":-0.77,"freq":"30MHz","label":"CasA"}
#cyg = {"flux":28062.87, "dir":"J2000 19h59m28.357s +40d44m02.097s","index":0.085,"freq":"30MHz","label":"CygA"}
#vir = {"flux":4201.61, "dir":"J2000 12h30m49.4233s +12d23m28.043s","index":-0.856,"freq":"30MHz","label":"VirA"} 
#tau = {"flux":2832.82, "dir":"J2000 05h34m31.971s +22d00m52.06s","index":-0.299,"freq":"30MHz","label":"TauA"}
...
scalef = np.zeros(len(ra_cal))
for i in range(len(ra_cal)):
    az, alt = eq2hrz(ra_cal[i],dec_cal[i],mjd,h,m,s,lat_ov,lon_ov)
    print az, alt
    scalef[i] = (np.sin(alt*(np.pi/180.0)))**1.6
    elev[i] = alt

#cas = {"flux":41093.75*scalef[0], "dir":"J2000 23h23m24.000s +58d48m54.000s","index":-0.77,"freq":"30MHz","label":"CasA"}
#cyg = {"flux":28062.87*scalef[1], "dir":"J2000 19h59m28.357s +40d44m02.097s","index":0.085,"freq":"30MHz","label":"CygA"}
#vir = {"flux":4201.61*scalef[2], "dir":"J2000 12h30m49.4233s +12d23m28.043s","index":-0.856,"freq":"30MHz","label":"VirA"} 
#tau = {"flux":2832.82*scalef[3], "dir":"J2000 05h34m31.971s +22d00m52.06s","index":-0.299,"freq":"30MHz","label":"TauA"}

flux_dict = {"cas":41093.75*scalef[0],"cyg":28062.87*scalef[1],"vir":4201.61*scalef[2],"tau":2832.82*scalef[3] }

#flux_dict = {"cas":41093.75*scalef[0],"vir":4201.61*scalef[2],"tau":2832.82*scalef[3] }
...
path2 = '.'
cal_file = path2+'calibrationtable.bcal'
...
os.system('rm -rf af_bp* afbp_image*')
split(vis=filename, outputvis='af_bp.ms', datacolumn='corrected')
#clean(vis='af_bp.ms', imagename='afbp_image_500', mode='mfs', imsize=[4096,4096],cell=['1.875arcmin'],niter=500, weighting= 'briggs', robust =0, stokes='I',gain = 0.05)
clean(vis='af_bp.ms', imagename='afbp_image', mode='mfs', imsize=[4096,4096],cell=['1.875arcmin'],niter=0, weighting= 'briggs', robust =0, stokes='I')


imname = 'afbp_image.image'
ia.open(imname)
B = ia.getchunk()
im  = B[:,:,0,0]
ia.close()
...
for src in ord_dict:
    ...
    if src[0] == 'cyg':
       ...
       #cl.addcomponent(flux=sjy, dir='J2000 19h59m28.357s +40d44m02.097s', index=-1.0022, spectrumtype='spectral index', freq='30MHz', label='CygA')
    ...
    clearcal('af_bp.ms', addmodel=True)
    ft('af_bp.ms/', complist='src.cl', usescratch=True)


    # Phase selfcal
    os.system('rm -rf phase_src.cal phase_src_revert.cal')
    gaincal(vis='af_bp.ms/', caltable='phase_src.cal',refant='34', uvrange='>10lambda', solnorm = True, calmode='p', gaintype='G')

    #applycal(vis = 'af_bp.ms', gaintable=['phase_src.cal'], interp='linear')

    # Amplitude selfcal
    os.system('rm -rf amp_src.cal amp_src_revert.cal')
    #gaincal(vis='af_bp.ms/', caltable='amp_src.cal',refant='34', uvrange='>12lambda', solnorm = True, calmode='a', gaintype='G')
    gaincal(vis='af_bp.ms/', caltable='amp_src.cal',refant='34', uvrange='>10lambda', solnorm = True, calmode='a', gaintype='G',gaintable=['phase_src.cal'])
    #applycal(vis = 'af_bp.ms', gaintable=['amp_src.cal'], interp='linear')
    applycal(vis = 'af_bp.ms', gaintable=['phase_src.cal','amp_src.cal'], interp='linear')

    # subtracting model from the corrected data
    uvsub('af_bp.ms/')
    split(vis='af_bp.ms', outputvis='af_bp_2.ms', datacolumn='corrected')    

    #clean(vis='af_bp.ms', imagename='af_sub', mode='mfs', imsize=[4096,4096],cell=['1.875arcmin'],niter=0,  weighting= 'natural', stokes='I')   
    # revert the calibration
    #os.system('./invert_table.py phase_src.cal')
    #os.system('./invert_table.py amp_src.cal')
    revert('phase_src.cal')
    revert('amp_src.cal')
    applycal(vis='af_bp_2.ms', gaintable=['amp_src_revert.cal','phase_src_revert.cal'], interp='linear')

    #split(vis='af_bp_2.ms', outputvis='af_bp_3.ms', datacolumn='corrected')

    #applycal(vis='af_bp_3.ms', gaintable=['phase_src_revert.cal'], interp='linear')
    
    
    # make the corrected data the data gain
    split(vis='af_bp_2.ms', outputvis='af_bp_3.ms', datacolumn='corrected')
    os.system('rm -rf af_bp.ms')
    os.system('mv af_bp_3.ms af_bp.ms')
    os.system('rm -rf af_bp_*')
    
   
# Imaging the subtracted data
os.system('rm -rf af_sub*')
clean(vis='af_bp.ms', imagename='af_sub_noiter', mode='channel',start = 0, width = 4,  imsize=[4096,4096],cell=['1.875arcmin'],niter=0, weighting= 'briggs', robust =0, stokes='I')
#clean(vis='af_bp.ms', imagename='af_sub_500', mode='mfs', imsize=[4096,4096],cell=['1.875arcmin'],niter=500,  weighting= 'briggs', robust =0, stokes='I',gain = 0.05)
