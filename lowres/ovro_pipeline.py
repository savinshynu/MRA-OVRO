...
def find_events(data2,time2,dur,sig_thresh,ngrid,psize,lat,lon):
    ...
    for i in range(data2.shape[0]):
        if i > 1:
           ...
           #vmin = sc(im_mask,0)
           #vmax = sc(im_mask,100)
           #plt.imshow(np.transpose(im_mask),cmap ='jet',vmin = vmin, vmax = vmax,origin ='lower')
           #plt.show()
           #print len(xar)
           if len(xar) > 0:
...
#path =sys.argv[1]
mjd =  'aug10'
path = '/leo/savin/ovro-lwa/low_res/avg_hdf5/'+mjd+'/*h.hdf5'
files = sorted(glob.glob(path))
path_2 = '/leo/savin/ovro-lwa/low_res/transients/'+mjd+'/'
...
for filename in files:
    ...
    #print ints
    xSize = hd[4] #size of images (x-axis)
    ySize = hd[4]  #size of images (x-axis)
    psize = hd[3]  #angular size of a pixel (at zenith)
...
if something:
    ...
    try:
        ...
        np.savetxt(path_2 + mjd + '_13s.txt',np.concatenate((np.reshape(RA5,(-1,1)),np.reshape(DEC5,(-1,1)),TIME5,np.reshape(SNR5,(-1,1))),1),'%10.3f')
    ...
    try:
        ...
        np.savetxt(path_2 + mjd+ '_26s.txt',np.concatenate((np.reshape(RA15,(-1,1)),np.reshape(DEC15,(-1,1)),TIME15,np.reshape(SNR15,(-1,1))),1),'%10.3f')
    ...
    try:
        ...
        np.savetxt(path_2 + mjd+ '_65s.txt',np.concatenate((np.reshape(RA60,(-1,1)),np.reshape(DEC60,(-1,1)),TIME60,np.reshape(SNR60,(-1,1))),1),'%10.3f')
