...
im = np.mean(dset[228,:,:,:],axis = 2) #- np.mean(dset[223:226,:,:],axis = 0)
vmin = sp(im,0)
vmax = sp(im,100)
#plt.pcolormesh(np.transpose(im),cmap='jet',vmin = vmin,vmax =vmax)
#plt.show()
#plt.plot(range(109),dset[100,77,216,0,:])
#plt.show()

time = tset[228,:]
mjdar = np.ones(2)*time[0]
har = np.ones(2)*time[1]
mar = np.ones(2)*time[2]
sar = np.ones(2)*time[3]

print getcoord(np.array([123.5,100]),np.array([186.5,100]),size,psize,mjdar,har,mar,sar,lat_ovro,lon_ovro)
