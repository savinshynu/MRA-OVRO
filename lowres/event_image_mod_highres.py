...
def flag_narrow_rfi(capt):
    ...
    for filename1 in files1:
        ...
        np.save('nearcor',dat_img)
        #fill = np.where((dat_im_nohrz != 0))
        vmin = sc(dat_img,0.1)
        vmax = sc(dat_img,99.9)
        plt.pcolormesh(np.transpose(dat_img[1600:2600,1600:2500]),cmap='jet',vmin = vmin,vmax = vmax)
        #plt.imshow(np.transpose(dat_img),origin = 'lower', cmap='jet',vmin = vmin,vmax = vmax)
        plt.colorbar()
        plt.show()   
