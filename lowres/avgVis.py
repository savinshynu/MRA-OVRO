...
# Average the visibilities and add it to the first MS data set
...
for i,b in enumerate(data1.baselines):
    ...
    new_vis = numpy.array([(data1.XX.data[i]+data2.XX.data[i])/2.0,
                           (data1.XY.data[i]+data2.XY.data[i])/2.0,
                           (data1.YX.data[i]+data2.YX.data[i])/2.0,
                           (data1.YY.data[i]+data2.YY.data[i])/2.0])
